...
const projects = [
  {
    id: 1,
    title: "Customized Inventory Management Solution with Support and Installation Tickets Management System",
    description: "Customized Inventory Management for the client. He needed a system to manage his inventory along with support and installation tickets management. The system includes role based access control, items and category management, tracking items, ticket creation and management, and transaction features",
    status: "deployed",
    stage: "Production",
    progress: 100,
    tags: ["React", "Material-UI", "Node.js", "Express.js", "MongoDB", "JWT", "REST API", "tailwindcss"],
    complexity: "High",
...
"/projects/imsmain.webp",
 ...
 "/projects/31.png",
  ...
  },
  {
    ...
    currentStage: 3,
    images: [
      "/projects/design-tool-1.png",
      "/projects/design-tool-2.png",
      "/projects/design-tool-3.png",
    ]
  },
]
...
function ProjectCard({ project, projectIndex, onImageClick }: ProjectCardProps) {
        ...
        <div className="relative h-48 sm:h-56 md:h-64 overflow-hidden bg-slate-100 dark:bg-slate-900">
            ...
            >
              <Image
                ...
                onError={(e) => {
                  ...
                  target.src = `https://images.unsplash.com/photo-${1500000000000 + project.id}?w=800&h=600&fit=crop`;
                }}
              />
            </motion.div>
          </AnimatePresence>
          ...
          {project.images.length > 1 && (
            <>
              <motion.button
                onClick={prevImage}
                className="absolute left-2 top-1/2 -translate-y-1/2 p-2 rounded-full bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border border-slate-200 dark:border-slate-700 hover:bg-white dark:hover:bg-slate-700 transition-all shadow-lg z-10"
              ...
              <motion.button
                onClick={nextImage}
                className="absolute right-2 top-1/2 -translate-y-1/2 p-2 rounded-full bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border border-slate-200 dark:border-slate-700 hover:bg-white dark:hover:bg-slate-700 transition-all shadow-lg z-10"
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.9 }}
              >
          ...
          <motion.button
            ...
            className="absolute top-3 right-3 p-2 rounded-full bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border border-slate-200 dark:border-slate-700 hover:bg-white dark:hover:bg-slate-700 transition-all shadow-lg z-10 opacity-0 group-hover:opacity-100"
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.9 }}
          >
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {selectedProject && (
          <motion.div
            className="fixed inset-0 z-[1000] flex items-center justify-center my-auto p-4 bg-black/90 backdrop-blur-md"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={closeModal}
          >
            <motion.div
              className="relative w-full max-w-6xl max-h-[80vh] bg-white dark:bg-slate-900 rounded-2xl overflow-hidden shadow-2xl"
            ...
            >
              ...
              <motion.button
                onClick={closeModal}
                className="absolute top-4 right-4 z-20 p-2 rounded-full bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm border border-slate-200 dark:border-slate-700 hover:bg-white dark:hover:bg-slate-700 transition-all shadow-lg"
              ...
              {/* Image Slider */}
              <div className="relative h-[60vh] sm:h-[70vh] bg-slate-100 dark:bg-slate-900">
                <AnimatePresence mode="wait">
                  <motion.div
                    key={modalImageIndex}
                  ...
                  >
                    <Image
                      ...
                      onError={(e) => {
                        const target = e.target as HTMLImageElement;
                        target.src = `https://images.unsplash.com/photo-${1500000000000 + selectedProject.id}?w=1200&h=800&fit=crop`;
                      }}
                    />
                  </motion.div>
                </AnimatePresence>
                ...
                {selectedProject.images.length > 1 && (
                  <>
                    <motion.button
                      ...
                      className="absolute left-4 top-1/2 -translate-y-1/2 p-3 rounded-full bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm border border-slate-200 dark:border-slate-700 hover:bg-white dark:hover:bg-slate-700 transition-all shadow-xl z-10"
                    ...
                    <motion.button
                      ...
                      className="absolute right-4 top-1/2 -translate-y-1/2 p-3 rounded-full bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm border border-slate-200 dark:border-slate-700 hover:bg-white dark:hover:bg-slate-700 transition-all shadow-xl z-10"
                      whileHover={{ scale: 1.1, x: 5 }}
                      whileTap={{ scale: 0.9 }}
                    >
                      <ChevronRight size={28} className="text-slate-900 dark:text-white" />
                    </motion.button>
                  </>
                )}
              </div>
                    ...
                    {selectedProject.images.map((image, index) => (
                      ...
                      >
                        <Image
                          ...
                          onError={(e) => {
                            const target = e.target as HTMLImageElement;
                            target.src = `https://images.unsplash.com/photo-${1500000000000 + selectedProject.id + index}?w=200&h=200&fit=crop`;
                          }}
                        />
                      </motion.button>
                    ))}
